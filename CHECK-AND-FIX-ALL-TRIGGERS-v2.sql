...
-- STEP 4: Drop and recreate log_activity function
...
-- STEP 6: Create update_updated_at function if needed
...
-- STEP 8: Show final triggers
SELECT
  '=== TRIGGERS ON TASKS ===' as section,
  trigger_name,
  event_manipulation,
  action_timing
FROM information_schema.triggers
...
-- STEP 9: Test task creation and update
DO $$
DECLARE
  v_test_task_id UUID;
...
BEGIN
  ...
  IF v_log_count > 0 THEN
    RAISE NOTICE '✅ Activity logs working! Found % entries', v_log_count;
  ELSE
    RAISE NOTICE '⚠️  No activity logs found';
  END IF;
  ...
  ELSE
    RAISE NOTICE '❌ UPDATE FAILED! Task not at in_progress';
  END IF;
  ...
  RAISE NOTICE '';
  RAISE NOTICE '============================================================';
  RAISE NOTICE '✅ ALL DONE! Tasks can now be created and updated!';
  RAISE NOTICE '============================================================';
END $$;
