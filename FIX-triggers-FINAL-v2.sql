-- =====================================================
-- FIX TRIGGERS - FINAL VERSION V2
-- Fixed foreign key constraint issue on DELETE
-- =====================================================

-- Drop and recreate log_activity function with correct foreign key handling
...
BEGIN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    INSERT INTO public.activity_logs (
      project_id,
      task_id,  -- Set to NULL for tasks to avoid FK constraint violation
      user_id,
      action,
      entity_type,
      entity_id,
      ...
      NULL,  -- Can't reference the task_id since it's being deleted
      COALESCE(OLD.updated_by, OLD.created_by),  -- use updated_by if available, fallback to created_by
      'deleted',
      TG_TABLE_NAME,
      OLD.id,  -- entity_id still has the UUID for reference
      to_jsonb(OLD),
      NOW()
    );
  END IF;
...
BEGIN
  ...
  -- Check activity logs (before delete)
  ...
  -- Clean up (this will test the DELETE trigger)
  DELETE FROM public.tasks WHERE id = v_test_task_id;
  RAISE NOTICE '✓ Test task deleted (testing DELETE trigger)';

  -- Check activity logs (after delete)
  SELECT COUNT(*) INTO v_log_count
  FROM public.activity_logs
  WHERE entity_id = v_test_task_id AND entity_type = 'tasks';

  IF v_log_count >= 3 THEN
    RAISE NOTICE '✅ DELETE trigger working! Found % total entries (create + update + delete)', v_log_count;
  ELSE
    RAISE NOTICE '⚠️  Only found % entries (expected 3 with delete)', v_log_count;
  END IF;

  RAISE NOTICE '';
  RAISE NOTICE '============================================================';
  RAISE NOTICE '✅ ALL DONE! All triggers are working correctly!';
  RAISE NOTICE '============================================================';
  RAISE NOTICE '';
